...
class BinarySearchTree:
    ...
    def __setitem__(self, k, v):
        '''Calls the insert method in order to overload [] for assignment.'''

        self.insert(k, v)

    def retrieve(self, key):
        if self.root:
            res = self._retrieve(key, self.root)
            if res:
                return res.data
            else:
                return None
    ...
    def _retrieve(self, key, currentNode):
        if not currentNode:
            return None
        elif currentNode.key == key:
            return currentNode
        elif key < currentNode.key:
            return self._retrieve(key, currentNode.leftChild)
        elif key > currentNode.key:
            return self._retrieve(key, currentNode.rightChild)
